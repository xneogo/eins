...
type List struct {
	...
	cond   chan struct{}
	closed bool
	// count of established connection
	active int
	// clean stale items
...
func (p *List) Put(ctx context.Context, c io.Closer, forceClose bool) error {
	...
	if !p.closed && !forceClose {
		...
		if p.idles.Len() > p.conf.Idle { // 超过最大空闲数量，删除该item
...
// Stat return count of established connections, idle connections, and corresponding config count
func (p *List) Stat() (int, int, int, int) {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.conf.Active, p.conf.Idle, p.active, p.idles.Len()
}
