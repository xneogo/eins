...
func (m *ClientGrpc) newConn(addr string) (clientpool.RpcClientConn, error) {
	...
	if err != nil {
		colorlog.Errorf(context.Background(), "%s dial addr: %s failed, err: %v", fun, addr, err)
		return nil, err
	}
...
func deleteAddrFromConnPool(addr string, pool *clientpool.ClientPool) {
	...
	colorlog.Infof(context.Background(), "%s get change addr success", fun)
	pool.Mu.Lock()
	...
	if !ok {
		colorlog.Warnf(context.Background(), "%s value to connection pool false, key: %s", fun, addr)
		return
	}
	pool.Pool.Delete(addr)
	clientPool.Close()
	colorlog.Infof(context.Background(), "%s close client pool success, addr: %s", fun, addr)
}

// basic used to add data or info before invoke rpc call
func metaDataUnaryClientInterceptor() grpc.UnaryClientInterceptor {
	return func(ctx context.Context, method string,
		...
		colorlog.Infof(ctx, "================== RPC 客户端拦截开始 ==================")
		colorlog.Infof(ctx, "req is: %+v", req)
		colorlog.Infof(ctx, "called: %s", method)
		...
		// TODO tobe tested
		//  adapter = client；service run as server
		//  can`t get caller info after adapter is called.
		md.Set("caller-method", method)
		...
		colorlog.Infof(ctx, "================== RPC 客户端拦截结束 ==================")
		return nil
	}
}
