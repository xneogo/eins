 ...
 *　　   ┃┫┫  ┃┫┫
 *      ┗┻┛　 ┗┻┛
 @Time    : 2024/11/6 -- 11:34
 @Author  : 亓官竹 ❤️ MONEY
 @Copyright 2024 亓官竹
 @Description: server_http.go
*/
...
import (
	"fmt"
	"github.com/qiguanzhu/infra/gehirn/onebooter"
...
const (
	HttpHeaderKeyTraceID = "nerv-trace-id"
...
func (s *HttpServer) Use(middleware ...HandlerFunc) {
	s.Engine.Use(multiWrap(middleware...)...)
}

// GET is a shortcut for router.Handle("GET", path, handle).
func (s *HttpServer) GET(relativePath string, handlers ...HandlerFunc) {
	ws := append([]gin.HandlerFunc{pathHook(relativePath)}, multiWrap(handlers...)...)
	...
	return func(c *gin.Context) {
		...
		if path, exist := c.Get(RoutePath); exist {
			if fun, ok := path.(string); ok {
				group, serviceName := bootup.GetGroupAndService()
				gstat.GetAPIRequestCountMetric().With(xprom.LabelGroupName, group, xprom.LabelServiceName, serviceName, xprom.LabelAPI, fun).Inc()
				gstat.GetAPIRequestTimeMetric().With(xprom.LabelGroupName, group, xprom.LabelServiceName, serviceName, xprom.LabelAPI, fun).Observe(float64(dt / time.Millisecond))
			}
		}
	}
}
...
func NotFound() gin.HandlerFunc {
	return func(c *gin.Context) {
		nfResp := &gentity.ErrorResponseBody{
			Ret:  -1,
...
func multiWrap(handlers ...HandlerFunc) []gin.HandlerFunc {
